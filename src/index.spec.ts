...
describe('Validator', function () {
    ...
    // let validator = new Validator({logger: console})
    ...
    it('object-$subItem', async function () {
        assert(
            await validator.validate(
                { a: '12512', b: 1241 },
                {
    ...
    it('object-or', async function () {
        assert(
            !(await validator.validate(124124, {
                $or: [
                    (val) => (val < 10 ? '' : '>=10'),
                    (val) => (val > 10 ? '' : '<=10'),
                ],
    ...
    it('object-recursive', async function () {
        // add person rule
        validator.addRule('person', {
            name: 'truthyString',
            age: 'number',
            children$: 'array[person]$',
        })
        validator.addRule('people', 'array[person]')
        const data = [
            {
                name: '龙一',
                age: 55,
